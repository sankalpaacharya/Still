...
export async function updateExpenseAction(data: any) {
    ...
    if (amountChanged || accountChanged) {
      ...
      if (revertError) {
        return { error: true, message: `Failed to revert old account balance: ${revertError.message}` };
      }
      ...
      if (newBalance < 0) {
        ...
        return { error: true, message: "Insufficient balance in selected account" };
      }
      ...
      if (updateNewAccountError) {
        ...
        return { error: true, message: `Failed to update new account balance: ${updateNewAccountError.message}` };
      }
    }
